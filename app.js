    ...
    placeholder3: true    // 占位3 - 新增
        ...
        function renderChatMessages() {
            ...
            messages.forEach(function(msg, index) {
                ...
                if (msg.isImage && msg.imageData) {
                    // 图片消息：限制大小为120px（与表情包相同），保持纵横比，对齐头像
                    bubble.innerHTML = `
                        <div class="chat-avatar">${avatarContent}</div>
                        <img src="${msg.imageData}" alt="图片" style="max-width:120px;max-height:120px;width:auto;height:auto;border-radius:8px;display:block;">
                    `;
                    // 为图片消息添加特殊class
                    bubble.classList.add('image-message');
                } else if (msg.emojiUrl && (!msg.content || msg.content === '[表情包]')) {
                    // 纯表情包消息：隐藏气泡和头像，只显示图片
                    bubble.innerHTML = `<img src="${msg.emojiUrl}" alt="表情" style="max-width:120px;max-height:120px;width:auto;height:auto;border-radius:8px;display:block;">`;
                    // 为表情包消息添加特殊class
