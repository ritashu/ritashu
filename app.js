...
严格按照这个格式输出，系统会自动提取和清理这一行，用户看不到这个内容。`);
        ...
        function updateInputAreaPosition() {
            ...
            if (lib.classList.contains('show')) {
                // 获取表情库的实际高度
                const libHeight = lib.offsetHeight;
                // 计算需要向上移动的距离 = 表情库高度
                const translateDistance = libHeight;
                
                // 设置 transform 使输入框和工具栏紧挨着表情库
                inputArea.style.transform = `translateY(-${translateDistance}px)`;
                toolbar.style.transform = `translateY(-${translateDistance}px)`;
            }
        }
        ...
        function setupEmojiLibraryObserver() {
            ...
            // 使用 MutationObserver 监听 DOM 变化（表情项目的增加或减少）
