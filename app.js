    ...
    placeholder3: true    // 占位3 - 新增
        ...
        function initEventListeners() {
            ...
            if (isVivoBrowser) {
                ...
                // 禁用某些影响性能的CSS过渡
                const originalStyle = chatInputElement.getAttribute('style') || '';
                chatInputElement.setAttribute('style', originalStyle + '; transition: none;');
            ...
            if (btnCamera && toolbarFile) btnCamera.addEventListener('click', function() { toolbarFile.click(); });
            if (btnPhoto && toolbarFile) btnPhoto.addEventListener('click', function() { toolbarFile.click(); });
            if (toolbarFile) toolbarFile.addEventListener('change', function(e) { handleToolbarFileSelect(e.target.files); });
            ...
            messages.forEach(function(msg, index) {
                ...
                if (msg.replyTo) {
                    ...
                    if (replyMsg) {
                        ...
                        textContent += `<div style="padding:6px;margin-bottom:8px;border-left:3px solid #ddd;background:#f5f5f5;border-radius:4px;font-size:11px;color:#999;max-width:200px;cursor:pointer;" onclick="event.stopPropagation(); scrollToMessage('${msg.replyTo}');">
                ...
                if (msg.isImage) {
                    if (msg.imageData) {
                        // 有图片数据，显示实际图片（无气泡）
                        textContent = ``;
                        bubble.innerHTML = `<img src="${msg.imageData}" alt="图片" style="max-width:200px;max-height:200px;border-radius:8px;display:block;">`;
                    } else {
                        // 无图片数据，显示[图片]或描述
                        textContent += msg.content || '[图片]';
                    }
                } else if (msg.emojiUrl) {
                    // 如果只有表情包，则不显示气泡，直接显示表情图片
                    if (!msg.content || msg.content === '[表情包]') {
                        textContent = ``;
                        bubble.innerHTML = `<img src="${msg.emojiUrl}" alt="表情" style="max-width:120px;max-height:120px;border-radius:8px;display:block;">`;
                    } else {
                        // 如果还有文字，则显示气泡和表情包
                        textContent += `<img src="${msg.emojiUrl}" alt="" style="max-width:100px;max-height:100px;border-radius:6px;display:block;margin-bottom:6px;">`;
                ...
                if (AppState.isSelectMode) {
                    ...
                    bubble.addEventListener('touchstart', (e) => {
                        ...
                        longPressTimer = setTimeout(() => {
                            if (touchStarted) {
                                e.preventDefault();  // 只在长按触发时才阻止默认行为
        ...
        function replyMessage(msgId) {
            ...
            if (!chatInput || !chatToolbar) return;
            ...
            if (msg.emojiUrl) {
                ...
                if (msg.content.length > 50) summary += '...';
            }
            const author = msg.type === 'sent' ? AppState.user.name : AppState.currentChat.name;
            
            // 移除已存在的引用显示
            const existingReplyBar = document.getElementById('reply-bar');
            ...
            replyContent.innerHTML = `
                <div style="font-size: 12px; color: #999; margin-bottom: 4px;">回复 <strong>${author}</strong></div>
                <div style="font-size: 13px; color: #333; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${summary}</div>
            `;
            ...
            closeBtn.addEventListener('click', () => {
                replyBar.remove();
                delete chatInput.dataset.replyToId;
            });
            ...
            // 将引用区域插入到聊天工具栏的最开始
            chatToolbar.insertBefore(replyBar, chatToolbar.firstChild);
        ...
        function handleToolbarFileSelect(files) {
            if (!files || files.length === 0) return;
            if (!AppState.currentChat) {
            ...
            modal.innerHTML = `
                <div class="emoji-mgmt-content" style="max-width:300px;background:#fff;border-radius:12px;overflow:hidden;">
                    ...
                    <div style="padding:12px;border-top:1px solid #e8e8e8;display:flex;gap:8px;justify-content:flex-end;">
                        ...
                        <button onclick="sendPhotoWithDescription('${imageData}');" style="padding:8px 16px;border:none;border-radius:4px;background:#000;color:#fff;cursor:pointer;font-size:13px;">发送</button>
                    </div>
                </div>
            `;
        ...
        function sendPhotoWithDescription(imageData) {
...
严格按照这个格式输出，系统会自动提取和清理这一行，用户看不到这个内容。`);
        ...
        function renderEmojiGroups(context) {
            ...
            groupsBar.style.cssText = 'display:flex;gap:8px;width:100%;overflow-x:auto;';
            ...
            AppState.emojiGroups.forEach(group => {
                ...
                groupsBar.appendChild(tag);
            });
            
            bar.innerHTML = '';
            bar.appendChild(groupsBar);
            ...
            modal.innerHTML = `
                <div class="emoji-mgmt-content" style="max-width:500px;max-height:90vh;overflow-y:auto;">
                    ...
                    <div style="padding:16px;">
                        ...
                        <!-- 角色人设 -->
                        <div style="margin-bottom:16px;">
                            <label style="display:block;font-size:13px;color:#666;margin-bottom:6px;font-weight:600;">角色人设/设定</label>
        ...
        function openCharacterMindState(chat) {
            ...
            let content = `
                <div class="emoji-mgmt-content" style="max-width:400px;background:#f5f5f5;">
                    <div style="padding:16px;border-bottom:1px solid #ddd;display:flex;justify-content:space-between;align-items:center;background:#fff;">
            ...
            content += `
                    ...
                    <div style="padding:12px;background:#fff;border-top:1px solid #ddd;display:flex;gap:8px;">
        ...
        function showCharacterMindHistory(charId) {
            ...
            let content = `
                <div class="emoji-mgmt-content" style="max-width:400px;background:#f5f5f5;max-height:70vh;overflow-y:auto;">
                    <div style="padding:16px;border-bottom:1px solid #ddd;display:flex;justify-content:space-between;align-items:center;background:#fff;position:sticky;top:0;">
